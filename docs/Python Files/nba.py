...
def get_players_in_season(season_year, scraper, output_file=None):
    print(f"\n[*] Fetching players from the {season_year} NBA season...")
    
    # Set default output file to current directory if not specified
    if output_file is None:
        output_file = os.path.join(os.getcwd(), f"nba_{season_year}_players.txt")
    ...
    try:
        ...
        output_lines = [f"NBA {season_year} Season - Player List ({len(df)} entries):\n"]
...
if __name__ == "__main__":
    try:
        year = int(input("Enter the NBA season year (e.g., 2023): "))
