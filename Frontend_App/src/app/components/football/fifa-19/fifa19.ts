...
@Component({
  selector: 'app-fifa-19',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa19.html',
  styleUrls: ['./fifa19.css'],
})
export class FIFA19Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA19Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA19Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA19')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA19Rating?.overallRating ?? 0;
            valueB = b.fifA19Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA19Rating?.club ?? '';
            valueB = b.fifA19Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA19Rating?.position ?? '';
            valueB = b.fifA19Rating?.position ?? '';
            break;
        }
