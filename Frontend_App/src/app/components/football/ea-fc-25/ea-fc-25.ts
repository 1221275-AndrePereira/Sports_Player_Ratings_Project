...
@Component({
  selector: 'app-ea-fc-25',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ea-fc-25.html',
  styleUrls: ['./ea-fc-25.css'],
})
export class EAFC25Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadEAFC25Players();
    }
  
    // ====================== Load Data ======================
    loadEAFC25Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('EAFC25')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.eafC25Rating?.overallRating ?? 0;
            valueB = b.eafC25Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.eafC25Rating?.club ?? '';
            valueB = b.eafC25Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.eafC25Rating?.position ?? '';
            valueB = b.eafC25Rating?.position ?? '';
            break;
        }
