...
@Component({
  selector: 'app-fifa-13',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa13.html',
  styleUrls: ['./fifa13.css'],
})
export class FIFA13Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA13Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA13Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA13')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA13Rating?.overallRating ?? 0;
            valueB = b.fifA13Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA13Rating?.club ?? '';
            valueB = b.fifA13Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA13Rating?.position ?? '';
            valueB = b.fifA13Rating?.position ?? '';
            break;
        }
