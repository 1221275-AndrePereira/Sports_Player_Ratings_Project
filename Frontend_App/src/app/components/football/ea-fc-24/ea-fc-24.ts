...
@Component({
  selector: 'app-ea-fc-24',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ea-fc-24.html',
  styleUrls: ['./ea-fc-24.css'],
})
export class EAFC24Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadEAFC24Players();
    }
  
    // ====================== Load Data ======================
    loadEAFC24Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('EAFC24')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.eafC24Rating?.overallRating ?? 0;
            valueB = b.eafC24Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.eafC24Rating?.club ?? '';
            valueB = b.eafC24Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.eafC24Rating?.position ?? '';
            valueB = b.eafC24Rating?.position ?? '';
            break;
        }
