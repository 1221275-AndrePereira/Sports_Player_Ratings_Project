...
@Component({
  selector: 'app-fifa-07',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa07.html',
  styleUrls: ['./fifa07.css'],
})
export class FIFA07Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA07Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA07Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA07')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA07Rating?.overallRating ?? 0;
            valueB = b.fifA07Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA07Rating?.club ?? '';
            valueB = b.fifA07Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA07Rating?.position ?? '';
            valueB = b.fifA07Rating?.position ?? '';
            break;
        }
