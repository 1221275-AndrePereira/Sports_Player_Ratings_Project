...
@Component({
  selector: 'app-fifa-2002',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa2002.html',
  styleUrls: ['./fifa2002.css'],
})
export class FIFA2002Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA2002Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA2002Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA2002')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA2002Rating?.overallRating ?? 0;
            valueB = b.fifA2002Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA2002Rating?.club ?? '';
            valueB = b.fifA2002Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA2002Rating?.position ?? '';
            valueB = b.fifA2002Rating?.position ?? '';
            break;
        }
