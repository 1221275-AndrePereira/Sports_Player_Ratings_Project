...
@Component({
  selector: 'app-fifa-16',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa16.html',
  styleUrls: ['./fifa16.css'],
})
export class FIFA16Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA16Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA16Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA16')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA16Rating?.overallRating ?? 0;
            valueB = b.fifA16Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA16Rating?.club ?? '';
            valueB = b.fifA16Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA16Rating?.position ?? '';
            valueB = b.fifA16Rating?.position ?? '';
            break;
        }
