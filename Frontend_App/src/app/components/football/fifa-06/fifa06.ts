...
@Component({
  selector: 'app-fifa-06',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa06.html',
  styleUrls: ['./fifa06.css'],
})
export class FIFA06Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA06Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA06Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA06')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA06Rating?.overallRating ?? 0;
            valueB = b.fifA06Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA06Rating?.club ?? '';
            valueB = b.fifA06Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA06Rating?.position ?? '';
            valueB = b.fifA06Rating?.position ?? '';
            break;
        }
