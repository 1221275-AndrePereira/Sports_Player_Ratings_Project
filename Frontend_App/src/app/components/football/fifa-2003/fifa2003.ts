...
@Component({
  selector: 'app-fifa-2003',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa2003.html',
  styleUrls: ['./fifa2003.css'],
})
export class FIFA2003Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA2003Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA2003Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA2003')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA2003Rating?.overallRating ?? 0;
            valueB = b.fifA2003Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA2003Rating?.club ?? '';
            valueB = b.fifA2003Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA2003Rating?.position ?? '';
            valueB = b.fifA2003Rating?.position ?? '';
            break;
        }
