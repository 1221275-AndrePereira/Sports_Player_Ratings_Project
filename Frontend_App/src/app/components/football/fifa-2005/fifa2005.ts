...
@Component({
  selector: 'app-fifa-2005',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa2005.html',
  styleUrls: ['./fifa2005.css'],
})
export class FIFA2005Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA2005Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA2005Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA2005')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA2005Rating?.overallRating ?? 0;
            valueB = b.fifA2005Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA2005Rating?.club ?? '';
            valueB = b.fifA2005Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA2005Rating?.position ?? '';
            valueB = b.fifA2005Rating?.position ?? '';
            break;
        }
