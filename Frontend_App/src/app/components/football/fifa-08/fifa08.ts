...
@Component({
  selector: 'app-fifa-08',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa08.html',
  styleUrls: ['./fifa08.css'],
})
export class FIFA08Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA08Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA08Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA08')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA08Rating?.overallRating ?? 0;
            valueB = b.fifA08Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA08Rating?.club ?? '';
            valueB = b.fifA08Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA08Rating?.position ?? '';
            valueB = b.fifA08Rating?.position ?? '';
            break;
        }
