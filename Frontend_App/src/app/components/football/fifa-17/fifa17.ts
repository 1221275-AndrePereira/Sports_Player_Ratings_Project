...
@Component({
  selector: 'app-fifa-17',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa17.html',
  styleUrls: ['./fifa17.css'],
})
export class FIFA17Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA17Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA17Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA17')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA17Rating?.overallRating ?? 0;
            valueB = b.fifA17Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA17Rating?.club ?? '';
            valueB = b.fifA17Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA17Rating?.position ?? '';
            valueB = b.fifA17Rating?.position ?? '';
            break;
        }
