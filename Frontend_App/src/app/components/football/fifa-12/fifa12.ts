...
@Component({
  selector: 'app-fifa-12',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa12.html',
  styleUrls: ['./fifa12.css'],
})
export class FIFA12Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA12Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA12Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA12')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA12Rating?.overallRating ?? 0;
            valueB = b.fifA12Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA12Rating?.club ?? '';
            valueB = b.fifA12Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA12Rating?.position ?? '';
            valueB = b.fifA12Rating?.position ?? '';
            break;
        }
