...
@Component({
  selector: 'app-fifa-20',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa20.html',
  styleUrls: ['./fifa20.css'],
})
export class FIFA20Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA20Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA20Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA20')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA20Rating?.overallRating ?? 0;
            valueB = b.fifA20Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA20Rating?.club ?? '';
            valueB = b.fifA20Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA20Rating?.position ?? '';
            valueB = b.fifA20Rating?.position ?? '';
            break;
        }
