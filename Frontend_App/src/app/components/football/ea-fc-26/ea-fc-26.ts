...
@Component({
  selector: 'app-ea-fc-26',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ea-fc-26.html',
  styleUrls: ['./ea-fc-26.css'],
})
export class EAFC26Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadEAFC26Players();
    }
  
    // ====================== Load Data ======================
    loadEAFC26Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('EAFC26')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.eafC26Rating?.overallRating ?? 0;
            valueB = b.eafC26Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.eafC26Rating?.club ?? '';
            valueB = b.eafC26Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.eafC26Rating?.position ?? '';
            valueB = b.eafC26Rating?.position ?? '';
            break;
        }
