...
@Component({
  selector: 'app-fifa-21',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa21.html',
  styleUrls: ['./fifa21.css'],
})
export class FIFA21Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA21Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA21Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA21')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA21Rating?.overallRating ?? 0;
            valueB = b.fifA21Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA21Rating?.club ?? '';
            valueB = b.fifA21Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA21Rating?.position ?? '';
            valueB = b.fifA21Rating?.position ?? '';
            break;
        }
