...
@Component({
  selector: 'app-fifa-14',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa14.html',
  styleUrls: ['./fifa14.css'],
})
export class FIFA14Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA14Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA14Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA14')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA14Rating?.overallRating ?? 0;
            valueB = b.fifA14Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA14Rating?.club ?? '';
            valueB = b.fifA14Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA14Rating?.position ?? '';
            valueB = b.fifA14Rating?.position ?? '';
            break;
        }
