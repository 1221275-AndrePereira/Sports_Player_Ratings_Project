...
@Component({
  selector: 'app-fifa-2004',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa2004.html',
  styleUrls: ['./fifa2004.css'],
})
export class FIFA2004Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA2004Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA2004Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA2004')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA2004Rating?.overallRating ?? 0;
            valueB = b.fifA2004Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA2004Rating?.club ?? '';
            valueB = b.fifA2004Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA2004Rating?.position ?? '';
            valueB = b.fifA2004Rating?.position ?? '';
            break;
        }
