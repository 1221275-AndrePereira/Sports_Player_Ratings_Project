...
@Component({
  selector: 'app-fifa-22',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa22.html',
  styleUrls: ['./fifa22.css'],
})
export class FIFA22Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA22Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA22Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA22')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA22Rating?.overallRating ?? 0;
            valueB = b.fifA22Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA22Rating?.club ?? '';
            valueB = b.fifA22Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA22Rating?.position ?? '';
            valueB = b.fifA22Rating?.position ?? '';
            break;
        }
