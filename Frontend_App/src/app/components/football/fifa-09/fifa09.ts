...
@Component({
  selector: 'app-fifa-09',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa09.html',
  styleUrls: ['./fifa09.css'],
})
export class FIFA09Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA09Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA09Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA09')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA09Rating?.overallRating ?? 0;
            valueB = b.fifA09Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA09Rating?.club ?? '';
            valueB = b.fifA09Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA09Rating?.position ?? '';
            valueB = b.fifA09Rating?.position ?? '';
            break;
        }
