...
@Component({
  selector: 'app-fifa-15',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa15.html',
  styleUrls: ['./fifa15.css'],
})
export class FIFA15Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA15Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA15Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA15')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA15Rating?.overallRating ?? 0;
            valueB = b.fifA15Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA15Rating?.club ?? '';
            valueB = b.fifA15Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA15Rating?.position ?? '';
            valueB = b.fifA15Rating?.position ?? '';
            break;
        }
