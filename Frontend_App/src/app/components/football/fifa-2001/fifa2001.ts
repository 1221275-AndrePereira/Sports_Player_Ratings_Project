...
@Component({
  selector: 'app-fifa-2001',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa2001.html',
  styleUrls: ['./fifa2001.css'],
})
export class FIFA2001Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA2001Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA2001Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA2001')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA2001Rating?.overallRating ?? 0;
            valueB = b.fifA2001Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA2001Rating?.club ?? '';
            valueB = b.fifA2001Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA2001Rating?.position ?? '';
            valueB = b.fifA2001Rating?.position ?? '';
            break;
        }
