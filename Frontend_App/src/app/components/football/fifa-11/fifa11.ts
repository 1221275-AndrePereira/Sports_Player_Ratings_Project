...
@Component({
  selector: 'app-fifa-11',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa11.html',
  styleUrls: ['./fifa11.css'],
})
export class FIFA11Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA11Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA11Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA11')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA11Rating?.overallRating ?? 0;
            valueB = b.fifA11Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA11Rating?.club ?? '';
            valueB = b.fifA11Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA11Rating?.position ?? '';
            valueB = b.fifA11Rating?.position ?? '';
            break;
        }
