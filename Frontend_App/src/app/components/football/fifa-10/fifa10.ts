...
@Component({
  selector: 'app-fifa-10',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './fifa10.html',
  styleUrls: ['./fifa10.css'],
})
export class FIFA10Component implements OnInit {
    ...
    ngOnInit(): void {
      this.loadFIFA10Players();
    }
  
    // ====================== Load Data ======================
    loadFIFA10Players() {
      this.isLoading = true;
      this.footballPlayerService.getFootballPlayersByGame('FIFA10')
    ...
    sortBy(column: 'playerName' | 'country' | 'rating' | 'club' | 'position') {
      ...
      this.filteredFootballPlayers.sort((a, b) => {
        ...
        switch (column) {
          ...
          case 'rating':
            valueA = a.fifA10Rating?.overallRating ?? 0;
            valueB = b.fifA10Rating?.overallRating ?? 0;
            break;

          case 'club':
            valueA = a.fifA10Rating?.club ?? '';
            valueB = b.fifA10Rating?.club ?? '';
            break;

          case 'position':
            valueA = a.fifA10Rating?.position ?? '';
            valueB = b.fifA10Rating?.position ?? '';
            break;
        }
